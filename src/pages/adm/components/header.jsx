      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            ...
            <span className="font-bold text-xl ml-3 hidden md:block">SIRLAB Manager</span>
          </div>
          <div className="relative" ref={menuRef}>
            ...
            >
              ...
              <div className="text-left hidden sm:block">
                ...
                <span className="text-xs text-slate-400 block">Perfil Admin</span>
              </div>
              <ChevronDown size={16} className={`text-slate-400 transition-transform duration-200 ${menuOpen ? 'rotate-180' : ''}`} />
            </button>
            {menuOpen && (
              <div className="origin-top-right absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg py-1 z-30 ring-1 ring-black ring-opacity-5 animate-fade-in-down">
                <button onClick={() => { onLogout(); setMenuOpen(false); }} className="flex items-center gap-3 w-full px-4 py-2 text-sm text-slate-700 hover:bg-red-50 hover:text-red-700 transition-colors">
                  <LogOut size={16} />
